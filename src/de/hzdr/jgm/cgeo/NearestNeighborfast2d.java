...
public class NearestNeighborfast2d
{
   //************************************ building the heap **********************************
   ...
   private static void binaryheapInPlace(double[][] original, int[] arr, int[] heap, int node, int height, int left, int right)
   {
      ...
      heap[node] = QuickSelect2d.selectKth(original, arr, index, height, left, right); // sort arr until k-th largest element and add to heap
   ...
   public static int NNcandidateindex(double[][] original, double[] point, int[] heap, int node, int height)
   {
      ...
      {
         if (QuickSelect2d.coordinatevalue(point,height) <= QuickSelect2d.coordinatevalue(original[heap[node]],height))
         {
    ...
    */
   public static double squaredEuclidean2D(double[] a, double[] b)
   {
   ...
   public static int NNindex(double[][] original, double[] point, int[] heap)
   {
      ...
      NN[0] = 0; // current best
      NN[1] = squaredEuclidean2D(point, original[heap[0]]);// initial NN distance      
   ...
   private static void NNindexrecursive(double[][] original, double[] point, int[] heap, int originalnode, int node, double[] NN)
   {
      ...
      { 
         ...
         double newdistance = squaredEuclidean2D(point, original[heap[node]]);
         if (newdistance < NN[1])
         {
            NN[0] = node; 
            NN[1] = newdistance;    
   ...
   {
      double sqdist = QuickSelect2d.coordinatevalue(point, height) - QuickSelect2d.coordinatevalue(original[heap[node]], height);
